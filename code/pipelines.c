...
sg_pipeline pk_unlit_tex_pip(const pk_pip_desc* desc) {
    ...
    pd.shader = sg_make_shader(pk_unlit_tex_shader_desc(sg_query_backend()));
    ...
    pd.layout.attrs[ATTR_pk_unlit_tex_position].format = SG_VERTEXFORMAT_FLOAT3;
    pd.layout.attrs[ATTR_pk_unlit_tex_normal].format = SG_VERTEXFORMAT_FLOAT3;
    pd.layout.attrs[ATTR_pk_unlit_tex_uv].format = SG_VERTEXFORMAT_FLOAT2;
...
sg_pipeline pk_texture_phong_pip(const pk_pip_desc* desc) {
    static sg_pipeline_desc pd = { 0 };
    pd.shader = sg_make_shader(pk_phong_tex_shader_desc(sg_query_backend()));
    pd.index_type = SG_INDEXTYPE_UINT32;
    pd.primitive_type = SG_PRIMITIVETYPE_TRIANGLES;
    pd.layout.buffers[0].stride = sizeof(pk_vertex_pnt);
    pd.layout.attrs[ATTR_pk_phong_tex_position].format = SG_VERTEXFORMAT_FLOAT3;
    pd.layout.attrs[ATTR_pk_phong_tex_normal].format = SG_VERTEXFORMAT_FLOAT3;
    pd.layout.attrs[ATTR_pk_phong_tex_uv].format = SG_VERTEXFORMAT_FLOAT2;
    pd.depth = desc->depth_state;
    pd.sample_count = desc->sample_count;
    if (desc->pixel_format != _SG_PIXELFORMAT_DEFAULT) {
        pd.colors[0].pixel_format = desc->pixel_format;
